 ...
 */
export function renderProgressBar(percentRemaining: number, width: number = 12): {
  filled: string;
  empty: string;
  width: number;
} {
 ...
 */
export function trafficLight(percentRemaining: number): TrafficLight {
 ...
 */
export function trafficLightColorKey(light: TrafficLight): "success" | "warning" | "error" {
  if (light === "ok") return "success";
  if (light === "warning") return "warning";
  return "error";
}

export function trafficLightIndicator(light: TrafficLight): string {
  if (light === "ok") return "●";
  if (light === "warning") return "●";
  return "●";
}
