...
async function tryLoad(
  filePath: string,
  source: LoadConfigSource,
): Promise<LoadAttempt | LoadFailure> {
  try {
...
export async function loadCardsConfig(): Promise<LoadConfigResult> {
  const errors: string[] = [];
  const candidates: Array<{ path: string; source: LoadConfigSource }> = [];
