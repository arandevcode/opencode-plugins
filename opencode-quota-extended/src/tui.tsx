    ...
    function acquire(sessionID: string) {
      ...
      const reload = () => {
        ...
        collectSnapshot(config).then((next) => {
          ...
          try { process.stderr.write(`[quota-cards] snapshot refresh failed: ${msg}\n`); } catch {}
        }).finally(() => {
          inFlight = false;
      ...
      const unsubs = [
        api.event.on("session.updated", (e) => { try { if (e?.properties?.info?.id === sessionID) scheduleRefresh(); } catch {} }),
        api.event.on("message.updated", (e) => { try { if (e?.properties?.info?.sessionID === sessionID) scheduleRefresh(); } catch {} }),
        api.event.on("message.removed", (e) => { try { if (e?.properties?.sessionID === sessionID) scheduleRefresh(); } catch {} }),
        api.event.on("tui.session.select", (e) => { try { if (e?.properties?.sessionID === sessionID) scheduleRefresh(); } catch {} }),
      ];
    ...
    api.slots.register({
      ...
      slots: {
        sidebar_content(_ctx: unknown, props: { session_id: string }) {
          return (
            <ErrorBoundary fallback={<text fg={api.theme.current.error}>Quota error</text>}>
              <SidebarContentView sessionID={props.session_id} />
            </ErrorBoundary>
          );
        },
      },
