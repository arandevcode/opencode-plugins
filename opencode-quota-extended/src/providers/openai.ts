...
export const openaiProvider: Provider = {
  ...
  async fetch(ctx: ProviderContext): Promise<ProviderQuota> {
    ...
    try {
      ...
      const primaryWindowData = rateLimit?.primary_window as Record<string, unknown> | undefined;
      const primary = windowFromData("5h", windowLabel(primaryWindowData, "5h"), primaryWindowData);
      if (primary) windows.push(primary);
      const secondaryWindowData = rateLimit?.secondary_window as Record<string, unknown> | undefined;
      const secondary = windowFromData("weekly", windowLabel(secondaryWindowData, "Weekly"), secondaryWindowData);
      if (secondary) windows.push(secondary);
      const codeReviewPrimary = windowFromData("code-review", windowLabel(codeReview?.primary_window as Record<string, unknown> | undefined, "Code Review"), codeReview?.primary_window as Record<string, unknown> | undefined);
