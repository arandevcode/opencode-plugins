...
function windowLabel(win, fallback) {
    ...
    if (seconds === 604_800)
        return "Weekly";
    if (seconds >= 2_592_000 && seconds <= 2_678_400)
        return "Monthly";
    return fallback;
}
...
export const openaiProvider = {
    ...
    async fetch(ctx) {
        ...
        try {
            ...
            const windows = [];
            const primaryWindowData = rateLimit?.primary_window;
            const primary = windowFromData("5h", windowLabel(primaryWindowData, "5h"), primaryWindowData);
            if (primary)
                windows.push(primary);
            const secondaryWindowData = rateLimit?.secondary_window;
            const secondary = windowFromData("weekly", windowLabel(secondaryWindowData, "Weekly"), secondaryWindowData);
            if (secondary)
                windows.push(secondary);
            const codeReviewPrimary = windowFromData("code-review", windowLabel(codeReview?.primary_window, "Code Review"), codeReview?.primary_window);
