...
- `setInterval` para alternar el color de la animación del borde (250ms, alterna entre `theme.warning` y `theme.error`)
- `setInterval` para detectar cambios de tema (250ms, `themeSignature()` polling)

## Colores según estado

Los colores vienen del tema activo de OpenCode (`api.theme.current`):

| Estado | Color del borde y badge | Texto del badge |
|--------|--------------------------|-----------------|
| Working (busy) | `theme.warning` ↔ `theme.error` (alterna cada 250ms) | `WORKING` |
...
|------|---------|---------------|
| a | `session_prompt`, `home_prompt` | `<box borderStyle="heavy" borderColor={color}>` envuelve al `api.ui.Prompt`. También añade un spinner vía `hint` y un badge `WORKING`/`IDLE` sobre el textarea. |
| b | `sidebar_title` | `<box borderStyle="heavy" borderColor={color}>` envuelve el título del sidebar. |
